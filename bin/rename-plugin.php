...
class RenamePluginCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!file_exists(__DIR__ . '/../src/JpmMartinSyliusSubscriptionPlugin.php')) {
            $io->warning('Plugin appears to be already renamed (JpmMartinSyliusSubscriptionPlugin.php not found)');
    ...
    private function renamePluginFiles(SymfonyStyle $io, array $names, bool $dryRun): void
    {
        ...
        $oldMainFile = __DIR__ . '/../src/JpmMartinSyliusSubscriptionPlugin.php';
        $newMainFile = __DIR__ . "/../src/{$names['fullClass']}.php";

        $oldExtensionFile = __DIR__ . '/../src/DependencyInjection/JpmMartinSyliusSubscriptionExtension.php';
        ...
        if (file_exists($oldMainFile)) {
            if ($dryRun) {
                $renamedFiles[] = "[DRY RUN] src/JpmMartinSyliusSubscriptionPlugin.php → src/{$names['fullClass']}.php";
            } else {
                if (!rename($oldMainFile, $newMainFile)) {
                    throw new \RuntimeException("Failed to rename {$oldMainFile}");
                }
                $renamedFiles[] = "src/JpmMartinSyliusSubscriptionPlugin.php → src/{$names['fullClass']}.php";
            }
        }

        if (file_exists($oldExtensionFile)) {
            if ($dryRun) {
                $renamedFiles[] = "[DRY RUN] src/DependencyInjection/JpmMartinSyliusSubscriptionExtension.php → src/DependencyInjection/{$names['extensionClass']}.php";
            } else {
                if (!rename($oldExtensionFile, $newExtensionFile)) {
                    throw new \RuntimeException("Failed to rename {$oldExtensionFile}");
                }
                $renamedFiles[] = "src/DependencyInjection/JpmMartinSyliusSubscriptionExtension.php → src/DependencyInjection/{$names['extensionClass']}.php";
            }
        }
    ...
    private function updateFileContents(SymfonyStyle $io, array $names, bool $dryRun): void
    {
        ...
        $replacements = [
            'jpmmartin_sylius_subscription_plugin_%kernel.environment%' => "{$names['db']}_%kernel.environment%",
            'JpmMartin\\SyliusSubscriptionPlugin' => "{$names['company']}\\{$names['plugin']}",
            'JpmMartinSyliusSubscriptionPlugin' => $names['fullClass'],
            'JpmMartinSyliusSubscriptionExtension' => $names['extensionClass'],
            '@JpmMartinSyliusSubscriptionPlugin' => "@{$names['fullClass']}",
            'Tests\\JpmMartin\\SyliusSubscriptionPlugin' => "Tests\\{$names['company']}\\{$names['plugin']}",
            'jpmmartin_sylius_subscription_plugin' => $names['db'],
            'jpmmartin_sylius_subscription' => $names['configKey'],
        ];
    ...
    private function updateComposerJson(SymfonyStyle $io, string $packageName, string $description, array $names, bool $dryRun): void
    {
        ...
        unset($composer['autoload']['psr-4']['JpmMartin\\SyliusSubscriptionPlugin\\']);
        $composer['autoload']['psr-4']["{$names['company']}\\{$names['plugin']}\\"] = 'src/';

        unset($composer['autoload-dev']['psr-4']['Tests\\JpmMartin\\SyliusSubscriptionPlugin\\']);
    ...
    private function checkRemainingReferences(SymfonyStyle $io): void
    {
        ...
        $patterns = ['Acme', 'SyliusExample', 'jpmmartin_sylius_subscription'];
        $found = [];
