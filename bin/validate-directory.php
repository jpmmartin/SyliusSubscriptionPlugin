...
if ($isSyliusOfficial) {
    if (!preg_match('/^[A-Z][a-zA-Z0-9]*Plugin$/', $dirName) || str_starts_with($dirName, 'Sylius')) {
        ...
        fwrite(STDERR, "    SYLIUS=1 composer create-project jpmmartin/sylius-subscription-plugin MailerLitePlugin\n");
        fwrite(STDERR, "\n");
    ...
    if (!preg_match('/^Sylius[A-Z][a-zA-Z0-9]*Plugin$/', $dirName)) {
        ...
        fwrite(STDERR, "\n");
        fwrite(STDERR, "  Example:\n");
        fwrite(STDERR, "    composer create-project jpmmartin/sylius-subscription-plugin SyliusWishlistPlugin\n");
        fwrite(STDERR, "\n");
