...
/** The admin form of a plan. Its variant is set by the route it is created from, never edited here. */
final class SubscriptionPlanType extends AbstractResourceType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            // Editable while creating, locked afterwards: the code identifies the plan to integrations.
            ->addEventSubscriber(new AddCodeFormSubscriber(null, ['label' => 'sylius.ui.code']))
            ->add('name', TextType::class, [
                'label' => 'sylius.ui.name',
            ])
            // A blank integer would reach the typed setter as null and fail the request; as 0 it
            // fails validation instead, with the message the administrator needs.
    ...
    {
        return 'jpm_martin_sylius_subscription_subscription_plan';
    }
}
