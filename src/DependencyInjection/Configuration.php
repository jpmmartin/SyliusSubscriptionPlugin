        ...
        $rootNode
            ...
            ->children()
                ...
                ->scalarNode('suspend_after_failed_cycles')
                    ...
                    ->end()
                ->end()
                ->scalarNode('renewal_notice_days')
                    ->info('Days before a scheduled cycle at which the RenewalUpcoming event is published, once per cycle, by the cycles command; null never publishes it. The plugin sends no email: listen to the event to tell the customer.')
                    ->defaultValue(3)
                    ->validate()
                        ->ifTrue(static fn (mixed $value): bool => null !== $value && (!\is_int($value) || $value < 1))
                        ->thenInvalid('The days before a renewal at which it is announced must be a positive integer, or null to never announce it; got %s.')
                    ->end()
                ->end()
                ->scalarNode('customer_payment_wait_minutes')
                    ->info('Minutes a retry waits for a payment its customer is making of the same renewal order, on the store\'s order payment page, so the renewal is not charged twice. Counted from the payment request\'s last change: one left untouched longer is taken as given up, and the retry goes ahead. Only gateways that use Sylius\'s payment requests can be waited for.')
                    ->defaultValue(60)
                    ->validate()
                        ->ifTrue(static fn (mixed $value): bool => !\is_int($value) || $value < 1)
                        ->thenInvalid('The minutes a retry waits for a customer\'s payment must be a positive integer; got %s.')
                    ->end()
                ->end()
                ->scalarNode('max_consecutive_skips')
                    ->info('Renewals in a row a customer may skip; null, the default, sets no limit. Only the skipped cycles right before the open one count: a paid, failed or otherwise cancelled cycle ends the run.')
                    ->defaultNull()
                    ->validate()
                        ->ifTrue(static fn (mixed $value): bool => null !== $value && (!\is_int($value) || $value < 1))
                        ->thenInvalid('The renewals in a row a customer may skip must be a positive integer, or null for no limit; got %s.')
                    ->end()
                ->end()
